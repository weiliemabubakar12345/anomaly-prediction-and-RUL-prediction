...
out_dir = r'D:\Springer_Nature_LaTeX_Template\Paper 1 _ Preventive Maintenance\results\CNN_v2'
...
print(f'CNN anomalous test windows: {len(anomaly_idxs)}')
if len(anomaly_idxs):
    save_reconstruction_examples(X_test[:len(anomaly_idxs)], test_recon[:len(anomaly_idxs)], test_mse[:len(anomaly_idxs)], os.path.join(out_dir, 'anom_examples'), prefix='cnn_test', max_examples=min(5, len(anomaly_idxs)))
